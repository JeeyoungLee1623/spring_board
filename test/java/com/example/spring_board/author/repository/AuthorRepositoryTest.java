...
@DataJpaTest
// Test 용의 configure 를 사용하는 AutoConfigureTestDatabase.Replace 가 기본 설정
// 그런데, 기존의 configure(application.yml)을 사용하려면 NONE 으로 세팅
...
//    테스트 시나리오
//    테스트 데이터를 저장 시키고, 저장된 데이터를 다시 조회해서 테스트데이터와 비교하면
//    repository 의 save, find 가능이 정상인지 검증할 수 있다.

//    테스트 코드의 장점은 부족한 기능을 사전에 알아차릴 수가 있다.
...
//        조회를 해야하는데 id 를 모르므로 기본 메서드인 findById 를 사용할 수가 없으므로
//        repository 에 findByEmail 을 추가
        Author authorDB = authorRepository.findByEmail("hong@naver.com");


//        input 값과 조회한 값이 같은지를 비교해 보는 방법
//        방법1: print 해본다
...
//        방법2: 실행시 에러가나면 빨간불, 정상이면 파란불로 구분할 수 있도록
//        JUnit 의 기능 활용
//        JUnit 이란 spring 에서 테스트 용도의 tool 로 가장 많이 사용되는 라이브러리
